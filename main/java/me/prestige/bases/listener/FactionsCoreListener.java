...
public class FactionsCoreListener implements Listener {
    ...
    public FactionsCoreListener(final Bases plugin) {
        this.plugin = plugin;
    }
    ...
    public static boolean attemptBuild(final Entity entity, final Location location, final String denyMessage, final boolean isInteraction) {
        ...
        if(entity instanceof Player) {
            ...
            final Faction factionAt = Bases.getPlugin().getFactionManager().getFactionAt(location);
            if(!(factionAt instanceof ClaimableFaction)) {
                result = false;
            } else if(factionAt instanceof Raidable && ((Raidable) factionAt).isRaidable()) {
                result = true;
            }
            if(factionAt instanceof ColorFaction) {
                final ColorFaction playerFaction = Bases.getPlugin().getFactionManager().getColorFaction(player.getUniqueId());
    ...
    public static boolean canBuildAt(final Location from, final Location to) {
        final Faction toFactionAt = Bases.getPlugin().getFactionManager().getFactionAt(to);
        if(toFactionAt instanceof Raidable && !((Raidable) toFactionAt).isRaidable()) {
            final Faction fromFactionAt = Bases.getPlugin().getFactionManager().getFactionAt(from);
    ...
    private void handleMove(final PlayerMoveEvent event, final PlayerClaimEnterEvent.EnterCause enterCause) {
        ...
        if(toFaction instanceof CapturableFaction || fromFaction instanceof CapturableFaction) {
            final CapturableFaction capturableFaction = (CapturableFaction) (toFaction instanceof CapturableFaction ? toFaction : fromFaction);
    ...
    public void onPlayerPortal(final PlayerPortalEvent event) {
        if(event.getCause() == PlayerTeleportEvent.TeleportCause.NETHER_PORTAL) {
            ...
            if(fromFac.isSafezone() || plugin.getFactionManager().getFactionAt(to).isSafezone()) {
                event.setTo(to.getWorld().getSpawnLocation().add(0.5, 0.0, 0.5));
                event.useTravelAgent(false);
                player.sendMessage(ChatColor.WHITE + "You were teleported to the spawn of target world as you were in a safe-zone.");
                return;
            }
            if(event.useTravelAgent() && to.getWorld().getEnvironment() == World.Environment.NORMAL) {
                ...
                if(factionAt instanceof ClaimableFaction) {
                    final Faction playerFaction = this.plugin.getFactionManager().getPlayerFaction(player.getUniqueId());
                    if(playerFaction != null && playerFaction.equals(factionAt)) {
                        return;
                    }
                    player.sendMessage(ChatColor.WHITE + "Portal would have created portal in territory of " + factionAt.getDisplayName((CommandSender) player) + ChatColor.WHITE + '.');
                    event.setCancelled(true);
                }
            }
        }
            ...
            if(attacker != null) {
                ...
                if(playerFaction != null && (attackerFaction = this.plugin.getFactionManager().getPlayerFaction(attacker.getUniqueId())) != null) {
                    ...
                    if(attackerFaction.equals(playerFaction)) {
                        attacker.sendMessage(ChatColor.RED + "You cannot hurt faciton members");
                        event.setCancelled(true);
                    }
                }
            }
    ...
    public void onPlayerInteract(final PlayerInteractEvent event) {
        ...
        if(action == Action.RIGHT_CLICK_BLOCK) {
            ...
            if(!canBuild && !attemptBuild((Entity) event.getPlayer(), block.getLocation(), ChatColor.WHITE + "You cannot do this in the territory of %1$s" + ChatColor.WHITE + '.', true)) {
                event.setCancelled(true);
            }
        }
    }
    ...
    public void onBlockBreak(final BlockBreakEvent event) {
        boolean nullifyMessage = event.getBlock().getType() == Material.DIAMOND_ORE || event.getBlock().getType() == Material.GOLD_ORE;
        if(plugin.getFactionManager().getFactionAt(event.getBlock()) instanceof WarzoneFaction){
            nullifyMessage = true;
        }
        if(!attemptBuild((Entity) event.getPlayer(), event.getBlock().getLocation(), nullifyMessage ? null : ChatColor.WHITE + "You cannot build in the territory of %1$s" + ChatColor.WHITE + '.')) {
            event.setCancelled(true);
        }
    }
    ...
    public void onHangingDamageByEntity(final EntityDamageByEntityEvent event) {
        ...
        if(entity instanceof Hanging) {
            ...
            if(!attemptBuild((Entity) attacker, entity.getLocation(), ChatColor.WHITE + "You cannot build in the territory of %1$s" + ChatColor.WHITE + '.')) {
                event.setCancelled(true);
            }
        }
    }
